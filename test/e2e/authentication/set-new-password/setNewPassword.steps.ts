...
class SetNewPasswordSteps {
  ...
  private givenNonce(nonce: string, callback: CallbackStepDefinition): void {
    this.currentNonce = nonce;
    callback();
  };

  @given(/^'(.*)' is the id representing the user in the email link$/)
  private givenId(id: string, callback: CallbackStepDefinition): void {
    this.currentId = id;
    callback();
  };

  @when(/^using the link to the set new password page$/)
  private whenGetPage(callback: CallbackStepDefinition): void {
    this.setNewPasswordPageObject.getPage(this.currentId, this.currentNonce);
    callback();
  };

  @then(/^the set new password request is validated '(.*)'$/)
  private thenRequestIsValidated(valid: string, callback: CallbackStepDefinition): void {
  ...
  private givenUserClicksTheSetNewPasswordLink(callback: CallbackStepDefinition): void {
  ...
  private thenNewPasswordFormIsValidated(valid: string, callback: CallbackStepDefinition): void {
    let isValid = valid === 'true';
